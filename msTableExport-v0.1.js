...
mstableExport.prototype.CSV = function () {
    ...
    for (i = 0; i < trs.length; i++) {
        var tds = trs[i].querySelectorAll('td:not(.mste_no_export),th:not(.mste_no_export)');
...
mstableExport.prototype.print = function () {
    ...
    var head = trs[0].querySelectorAll('td:not(.mste_no_export),th:not(.mste_no_export)');
    data += '<thead>';
    data += '<tr>';
    ...
    for (var i = 1; i < trs.length; i++) {
        ...
        data += '<tr style="' + rs + ' ">';
        var tds = trs[i].querySelectorAll('td:not(.mste_no_export)');
